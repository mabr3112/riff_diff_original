...
def main(args):
    ...
    ensembles, index_layers = mpnn_design_and_esmfold(ensembles, prefix="round1", index_layers_to_reference=1, num_mpnn_seqs=args.num_mpnn_seqs, num_esm_inputs=args.num_esm_inputs, num_esm_outputs_per_input_backbone=args.num_esm_outputs_per_input_backbone, bb_rmsd_dir=diffusions, ref_pdb_dir=pdb_dir)
    for i in range(5):
        # run partial diffusion:
        ensembles, index_layers, diff_dir = run_partial_diffusion(ensembles, prefix=f"round{str(i)}_pd", index_layers_to_reference=index_layers, num_partial_diffusions=args.num_partial_diffusions, num_partial_diffusion_outputs=1, num_partial_timesteps=args.num_partial_diffusion_timesteps, ref_pdb_dir=pdb_dir)

    # rerun mpnn and predict with ESMFold:
    ensembles, index_layers = mpnn_design_and_esmfold(ensembles, prefix="round2", index_layers_to_reference=index_layers, num_mpnn_seqs=args.num_mpnn_seqs, num_esm_inputs=args.num_esm_inputs, num_esm_outputs_per_input_backbone=args.num_esm_outputs_per_input_backbone, bb_rmsd_dir=diff_dir, ref_pdb_dir=pdb_dir)

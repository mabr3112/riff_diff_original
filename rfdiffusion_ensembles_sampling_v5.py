...
def mpnn_design_and_af2(poses, prefix:str, index_layers_to_reference:int=0, num_mpnn_seqs:int=20, num_esm_inputs:int=8, num_esm_outputs_per_input_backbone:int=1, ref_pdb_dir:str=None, bb_rmsd_dir:str=None):
    '''AAA'''
    # Run MPNN and filter (by half)
    mpnn_designs = poses.mpnn_design(mpnn_options=f"--num_seq_per_target={num_mpnn_seqs} --sampling_temp=0.1", prefix=f"{prefix}_mpnn", fixed_positions_col="fixed_residues")
    mpnn_seqfilter = poses.filter_poses_by_score(args.num_esm_inputs, f"{prefix}_mpnn_score", prefix=f"{prefix}_mpnn_seqfilter", remove_layers=1)
    ...
    # Run ESMFold and calc bb_ca_rmsd, motif_ca_rmsd and motif_heavy RMSD
    poses.poses_df[f"{prefix}_af2_pose_opts"] = [f'--templates {diff_output}' for diff_output in poses.poses_df["rfdiffusion_location"].to_list()]
    esm_preds = poses.predict_sequences(run_AlphaFold2, prefix=f"{prefix}_af2", options="--msa-mode single_sequence --num-models 1", pose_options=poses.poses_df[f"{prefix}_af2_pose_opts"].to_list())
    esm_bb_ca_rmsds = poses.calc_bb_rmsd_dir(ref_pdb_dir=bb_rmsd_dir, metric_prefix=f"{prefix}_af2", ref_chains=["A"], pose_chains=["A"], remove_layers=1)
    esm_motif_rmsds = poses.calc_motif_bb_rmsd_dir(ref_pdb_dir=ref_pdb_dir, ref_motif=list(poses.poses_df["template_motif"]), target_motif=list(poses.poses_df["motif_residues"]), metric_prefix=f"{prefix}_af2_bb_ca", remove_layers=index_layers_to_reference+1)
    esm_motif_heavy_rmsds = poses.calc_motif_heavy_rmsd_dir(ref_pdb_dir=ref_pdb_dir, ref_motif=poses.poses_df["template_fixedres"].to_list(), target_motif=poses.poses_df["fixed_residues"].to_list(), metric_prefix=f"{prefix}_af2_catres", remove_layers=index_layers_to_reference+1)

    # Filter Redesigns based on confidence and RMSDs
    esm_comp_score = poses.calc_composite_score(f"{prefix}_af2_comp_score", [f"{prefix}_af2_top_plddt", f"{prefix}_af2_bb_ca_motif_rmsd"], [-1, 1])
    esm_filter = poses.filter_poses_by_score(num_esm_outputs_per_input_backbone, f"{prefix}_af2_comp_score", remove_layers=1, prefix=f"{prefix}_af2_filter", plot=[f"{prefix}_af2_comp_score", f"{prefix}_af2_top_plddt", f"{prefix}_af2_bb_ca_rmsd", f"{prefix}_af2_bb_ca_motif_rmsd", f"{prefix}_af2_catres_motif_heavy_rmsd"])
    
    # Plot Results
    if not os.path.isdir((plotdir := f"{poses.dir}/plots")): os.makedirs(plotdir, exist_ok=True)

    # ESM stats:
    cols = [f"{prefix}_af2_top_plddt", f"{prefix}_af2_bb_ca_rmsd", f"{prefix}_af2_bb_ca_motif_rmsd", f"{prefix}_af2_catres_motif_heavy_rmsd"]
    titles = ["AF2 pLDDT", "AF2 BB-Ca RMSD", "AF2 Motif-Ca RMSD", "Af2 Catres\nSidechain RMSD"]
    y_labels = ["pLDDT", "RMSD [\u00C5]", "RMSD [\u00C5]", "RMSD [\u00C5]"]
    dims = [(0,100), (0,15), (0,8), (0,8)]
    _ = plots.violinplot_multiple_cols(poses.poses_df, cols=cols, titles=titles, y_labels=y_labels, dims=dims, out_path=f"{plotdir}/{prefix}_esm_stats.png")
    return poses, index_layers_to_reference+1
...
def run_partial_diffusion(poses, prefix:str, index_layers_to_reference:int=2, num_partial_diffusions:int=5, num_partial_diffusion_outputs:int=4, num_partial_timesteps:int=15, ref_pdb_dir:str=None):
    ...
    #partdiff_sampling_filter2 = ensembles.filter_poses_by_score(num_partial_diffusion_outputs, f"{prefix}_partial_diffusion_comp_score", prefix=f"{prefix}_partial_diffusion_sampling_filter2", remove_layers=2, plot=[f"{prefix}_partial_diffusion_comp_score", f"{prefix}_partial_diffusion_plddt", f"{prefix}_partial_diffusion_template_bb_ca_motif_rmsd"])
...
def main(args):
    ...
    hal_sampling_filter2 = ensembles.filter_poses_by_score(args.num_mpnn_inputs, "rfdiffusion_comp_score", prefix="rfdiffusion_sampling_filter2", remove_layers=2, plot=["rfdiffusion_comp_score", "rfdiffusion_plddt", "rfdiffusion_template_bb_ca_motif_rmsd"])
    ...
    ensembles, index_layers = mpnn_design_and_af2(ensembles, prefix="round1", index_layers_to_reference=1, num_mpnn_seqs=args.num_mpnn_seqs, num_esm_inputs=args.num_esm_inputs, num_esm_outputs_per_input_backbone=args.num_esm_outputs_per_input_backbone, bb_rmsd_dir=diffusions, ref_pdb_dir=pdb_dir)
    
    # Plot Results
    if not os.path.isdir((plotdir := f"{ensembles.dir}/plots")): os.makedirs(plotdir, exist_ok=True)

    # RFdiffusion stats:
    #cols = ["rfdiffusion_plddt", "rfdiffusion_template_bb_ca_motif_rmsd", "partial_diffusion_plddt", "partial_diffusion_template_bb_ca_motif_rmsd"]
...
if __name__ == "__main__":
    ...
    argparser.add_argument("--num_rfdiffusion_outputs_per_input_backbone", type=str, default=2, help="Number of rfdiffusions that should be kept per input fragment.")

    # partial diffusion optiosn
    argparser.add_argument("--num_partial_diffusions", type=int, default=5, help="Number of partial diffusion runs per input backbone")
    argparser.add_argument("--num_partial_diffusion_timesteps", type=int, default=15, help="Number of partial diffusion timesteps")
    argparser.add_argument("--rfdiff_guide_scale", type=int, default=2, help="Value for potentials.guide_scale for RFDiffusion.")
    ...
    argparser.add_argument("--output_scoreterms", type=str, default="af2_top_plddt,af2_bb_ca_motif_rmsd", help="Scoreterms to use to filter ESMFolded PDBs to the final output pdbs. IMPORTANT: if you supply scoreterms, also supply weights and always check the filter output plots in the plots/ directory!")

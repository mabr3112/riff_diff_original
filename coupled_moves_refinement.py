...
def prepare_coupled_moves_relax_mpnn(output_dir, working_dir, cm_resultsdir, poses_df, occurence_cutoff, cycle_num):
    ...
    for index, row in poses_df.iterrows():
        ...
        for seqnum, var in variants_df.iterrows():
            ...
            fixed_pos = sorted(row['fixed_residues']['A'] + [int(pos) for pos in mutations])
...
def main(args):
    ...
    for index, row in coupled_moves.poses_df.iterrows():
        ...
        pre_cm_rx_options = f"-parser:script_vars cat_res={cat_res} motif_res={motif_res} -in:file:native {row['updated_reference_frags_location']}"
    ...
    for cycle_num in range(1, args.cycles + 1):
        ...
        #relax all poses to optimize rmsds
        ...
        coupled_moves.rosetta("rosetta_scripts.default.linuxgccrelease", options=pre_cm_rx_opts, pose_options=coupled_moves.poses_df['pre_cm_rx_opts'].to_list(), n=1, prefix=f'cycle_{cycle_num}_pre_cm_relax')
        ...
        coupled_moves.mpnn_design(mpnn_options=f"--num_seq_per_target={args.mpnn_nstruct} --sampling_temp={args.mpnn_temp}", prefix=f"cycle_{cycle_num}_cm_mpnn", fixed_positions_col=f"cycle_{cycle_num}_cm_fixed_positions", use_soluble_model=True)
    ...
    coupled_moves = clash_detection(poses=coupled_moves, ref_frags_col="updated_reference_frags_location", ref_motif_col="motif_residues", poses_motif_col="motif_residues", prefix="post_cm_af2", ligand_chain=args.ligand_chain, database_dir=args.database_dir, bb_clash_vdw_multiplier=args.bb_clash_vdw_multiplier, save_path_list=coupled_moves.poses_df['poses'].to_list())
    coupled_moves.poses_df.to_json(coupled_moves.scorefile)
...
if __name__ == "__main__":
    ...
    argparser.add_argument("--max_output_per_backbone", type=int, default=3, help="maximum number of output pdbs coming from identical starting structures (= number of structures predicted with alphafold)")
    ...
    argparser.add_argument("--use_reduced_motif", type=str, default=False, help="Only fix residues +- 1 around catalytic residues during trajectory.")
    ...
    #relax options
    argparser.add_argument("--relax_protocol", type=str, default='rosetta/coupled_moves_relax_new.xml', help="path to xmlfile that should be used for coupled moves relax runs")
    argparser.add_argument("--pre_cm_relax_protocol", type=str, default='rosetta/relax.xml', help="path to xmlfile that should be used for relax runs before running coupled moves")
    ...
    argparser.add_argument("--bb_clash_vdw_multiplier", type=float, default=0.8, help="Multiplier for VanderWaals radii for clash detection between backbone and ligand. Clash is detected if distance_between_atoms < (VdW_radius_atom1 + VdW_radius_atom2)*multiplier")
